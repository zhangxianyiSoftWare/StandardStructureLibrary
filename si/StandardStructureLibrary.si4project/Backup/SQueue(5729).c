...
{
    bool ret =slist_destory(&pqueue->queue);
    memset(&pqueue->queue,0,sizeof(SList_t) );
    pqueue->pfrist = NULL;
    pqueue->plast = NULL;
    free(pqueue);
    return ret;
...
{
    pQueueNode ret = deepCopyQueueNode(pqueue->pfrist->pnext);
    slist_del(&pqueue->queue, pqueue->pfrist->pnext);
