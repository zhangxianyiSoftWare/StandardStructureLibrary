...
pArray array_create( uint32 elem_size,bool (*freefunc)(void* elem) )
{
    ...
    array->alloc_num = 8;
    array->memery_size = array->alloc_num * array->elem_size ;
    array->freefn = freefunc;
    
    array->elems = Malloc(array->elem_size);
    assert(array->elems);
    return array;
}
...
/*****************************************************************************
*   Prototype    : array_control
*   Description  : search the array and operated it
*   Input        : pArray parray
*                  void (*func)(pArray parray,uint32 id)
*                  uint32 id
*   Output       : None
*   Return Value : bool
*   Calls        : 
...
*
*****************************************************************************/
bool array_control( pArray parray, void (*func)(pArray parray,uint32 id), uint32 id )
{
    for(int i = 0;i<parray->elem_num;++i)
    {
        void* addr = parray->elems + i * parray->elem_size;
